<!doctype html>
<html>
<head>
  <meta charset="utf-8"/>
  <title>Bourbon + Neat</title>
  <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0"/>
  <link rel="stylesheet" href="css/styles.css"/>
  <!--[if IE]>
      <link href="/stylesheets/ie.css" media="screen, projection" rel="stylesheet" type="text/css" />
  <![endif]-->
</head>

<body>

	<header>
		<nav>
			<ul>
				<li>Services</li>
				<li>About</li>
				<li>Sell your device</li>				
				<li>Contact us</li>
			</ul>
		</nav>
		<ul id="social-links">
			<li></li>
			<li></li>
			<li></li>
			<li></li>
		</ul>
		<a href="#" class="btn">Request repair</a>
		<a href="#" class="btn">(904) 310-0059</a>
	</header>
	
	<section id="carousel">
		<article>
			<h2>Welcome to Phone ResQ.</h2>
			<p>We repair and improve all of your handheld devices and home computers. Our hassle-free customer service is centered around your interests and the intention of getting you back online.</p>
			<a href="#">Contact us</a>
		</article>
		<article></article>
		<article></article>
	</section>

	<section id="services">
		<hgroup>
			<h1>Services</h1>
			<p>We’re here to serve you with all of your device-related needs. Whether your screen is cracked or you’re looking for assistance setting up a program on your computer, we’ve got you covered.</p>
		</hgroup>

		<div id="mouse-scroll"></div>

		<hr />

		<div id="repair-type">
			<article>
				<h2>iPhone repair</h2>
				<p>Whether you have the original iPhone or the latest model, we can take a look at your device and figure out what steps need to be made to get you back on the grid.</p>
			</article>
			<article>
				<h2>iPad repair</h2>
				<p>If you don’t have the time to make an appointment and have your tablet sent away to a third-party repair shop, consider our in-house services for all of your iPad needs.</p>
			</article>
			<article>
				<h2>iPod Repair</h2>
				<p>So your iPod is busted and you need to get back to jamming out on your favorite tunes. We get it.</p>
			</article>
			<article>
				<h2>Android Repair</h2>
				<p>We’re not just about Apple here. We work on Android devices as well. All devices and models are covered in the hands of our knowledgeable staff.</p>
			</article>
			<article>
				<h2>Laptop &amp; Desktop Repair</h2>
				<p>Doesn’t it feel like when your computer isn’t working, that life slows down? Let us take a look and we’ll get you up and running again in no time.</p>						
			</article>
			<article>
				<h2>Cell Phone unlocks</h2>
				<p>Interested in unlocking your phone? We’ve got the tools to safely and securely unlock almost any smartphone device.</p>
			</article>
		</div>
	</section><!-- End of repair type -->

	<section id="about">
		<div id="about-section-one">
			<div>
				<h2>Phone ResQ strives to create a new and unique experience for its customers:</h2>
				<p>A one-on-one interaction that redefines customer service geared toward consistency and ultimate satisfaction.</p>
				<p>We at Phone ResQ put our customers first and understand what it means to go a day or two without your device. When you put your device-related needs in our hands, we spring into action and work tirelessly to get your device running smoothly once again, so you can resume your life and work with ease.</p>
			</div>		
			<div id="bg-image"></div>			
		</div>

		<div id="about-section-two">
			<div>
				<h2>Phone ResQ strives to create a new and unique experience for its customers:</h2>
				<p>A one-on-one interaction that redefines customer service geared toward consistency and ultimate satisfaction.</p>
				<p>We at Phone ResQ put our customers first and understand what it means to go a day or two without your device. When you put your device-related needs in our hands, we spring into action and work tirelessly to get your device running smoothly once again, so you can resume your life and work with ease.</p>
			</div>		
			<div id="bg-image"></div>			
		</div>

		<div id="about-section-three">
			<div>
				<h2>Phone ResQ strives to create a new and unique experience for its customers:</h2>
				<p>A one-on-one interaction that redefines customer service geared toward consistency and ultimate satisfaction.</p>
				<p>We at Phone ResQ put our customers first and understand what it means to go a day or two without your device. When you put your device-related needs in our hands, we spring into action and work tirelessly to get your device running smoothly once again, so you can resume your life and work with ease.</p>
			</div>		
			<div id="bg-image"></div>
		</div>
	</section><!-- End of About -->

	<section id="sell">
		<div id="bg-image"></div>
		<div>
			<h2>Need to sell your iPhone, iPod, iPad, Mac?</h2>
			<p>No matter what the condition of your device is. We will buy it on the spot for cash.</p>
			<p>Please contact us for a quote! Use the contact form below or give us a call during business hours! 904-310-0059</p>
		</div>
	</section><!-- End of Sell -->

	<section id="shop">
		<hgroup>
			<h1>Shop</h1>
		</hgroup>			
		<p>Please find some products that are for sale or refurbished.</p>

		<article id="iPhone" class="item">
			<img src="" alt="" />
			<a href="#">Find out more</a>
		</article>

		<article id="samsung" class="item">
			<img src="" alt="" />
			<a href="#">Find out more</a>
		</article>

		<article id="iPad" class="item">
			<img src="" alt="" />
			<a href="#">Find out more</a>
		</article>		

		<article id="samsung-galaxy" class="item">
			<img src="" alt="" />
			<a href="#">Find out more</a>
		</article>

		<article id="accessories" class="item">
			<img src="" alt="" />
			<a href="#">Find out more</a>
		</article>
	</section><!-- End of shop -->

	<section id="contact">
		<hgroup>
			<h1>Contact us</h1>
		</hgroup>			
		<p>Please fill in all the fields below to send us a message online.</p>
		<form>
			<input type="text" id="name" />
			<input type="text" id="email" />
			<input type="text" id="phone" />
			<input type="text" id="message" />
			<input type="submit">
		</form>		
	</section><!-- End of contact -->
	<section id="get-in-touch"></section><!-- End of get int touch -->
	<section id="clients"></section><!-- End of clients -->

	
	<footer></footer>

	<script src="https://ajax.googleapis.com/ajax/libs/jquery/2.1.4/jquery.min.js"></script>
	<script>window.jQuery || document.write('<script src="scripts/jquery-1.6.2.min.js"><\/script>')</script>
	<script type="text/javascript" src="scripts/scripts.js"></script>
</body>
</html>